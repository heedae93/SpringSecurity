...
### ✅ 6단계 - Security 설정 완성
- `SecurityConfig` : 지금까지 만든 모든 클래스를 Spring Security에 등록하고 연결
  - CustomAuthenticationProvider 등록 (비밀번호 검증 담당)
  - JwtFilter를 FilterChain에 등록 (모든 요청에서 JWT 검사)
  - /auth/login 공개 경로 설정, 나머지 인증 필요
  - 세션 STATELESS, CSRF/FormLogin/HttpBasic 비활성화

### ✅ 7단계 - 로그인 API 완성
- `AuthService` : 로그인 처리 메서드를 선언하는 인터페이스
- `AuthServiceImpl` : AuthenticationManager에 인증 위임 후 JWT 발급
